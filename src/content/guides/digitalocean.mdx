...
tar -czvf nome_archivio.tar.gz file1 file2 file3
```
...
Per editare un file dal terminale:
```bash
...
Per visualizzare tutte le porte attive di un ip remoto
```bash
nmap -p 0-65535 indirizzo_ip
```

nmap va installato con:
```bash
sudo apt update
...
- un A record con `nome_dominio` che punta all'IP pubblico del server.
